...
public class ArmstrongNumber {
    public static void main(String[] args) {
        ...
        The program should ask for a number.number E.g. if we type 371, the program should print out:
    ...
    public static void armstrongNumber(int number) {
        ...
        int length = valueOfNumber.length();
        int sumOfDigits = 0;

        for (int i = 0; i < length; i++) {
            char a = valueOfNumber.charAt(i);
            int digit = Character.getNumericValue(a);

            sumOfDigits += Math.pow(digit, length);
        }
